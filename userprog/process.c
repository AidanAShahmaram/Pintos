...
static void start_process(void *func_args) {
  ...
  set_child_tid(cast_args->wait_struct, thread_current()->tid);
   ...
   /*palloc_free_page(file_name);
	...
	uintptr_t esp_val_1 = (uintptr_t) if_.esp;

	//if_.esp -= 16 - (esp_val_1 % 16);
	...
	//word-align the stack pointer
        //uintptr_t sp = (uintptr_t) if_.esp;
        //if_.esp -= sp % 4;                                                                                                                                             
        //if_.esp -= sizeof(void *);
        //*(void **)if_.esp = 0;

	
        //align to 16-bit boundary after stack pushes
        uintptr_t esp_val_2 = (uintptr_t) if_.esp;
	int diff = (esp_val_1 - esp_val_2);
        int misalignment = diff % 16;
        if(misalignment != 0){
            if_.esp -= 16 - misalignment;
        }
	...
	if(argc % 2 != 0){ 
	  if_.esp -=  (argc % 4) * 4;
        ...
        //push pointers to each argument
        for(int i = argc - 1; i >= 0; i--){
            if_.esp -= sizeof(char *);
            memcpy(if_.esp, &arg_ptrs[i], sizeof(char *));
        }
